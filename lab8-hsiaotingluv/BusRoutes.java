...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    final String header = "Search for: " + this.stop + " <-> " + name + ":\n" 
      + "From " + this.stop + "\n";

    CompletableFuture<String> result = CompletableFuture.completedFuture(header);

    for (BusService service : services.keySet()) {
      CompletableFuture<Set<BusStop>> stops = services.get(service);

      CompletableFuture<String> stopDesc = stops
        .thenApply(stop -> describeService(service, stop));

      result = result.thenCombine(stopDesc, (string1, string2) -> string1 + string2);
    }
    return result;
  }
