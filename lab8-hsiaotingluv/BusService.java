 ...
 */
class BusService extends CompletableFuture {
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    CompletableFuture<String> result = BusAPI.getBusStopsServedBy(serviceId);

    return result.thenApply(busStopsString -> {
      Scanner sc = new Scanner(busStopsString);
  ...
  public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
    return getBusStops()
      .thenApply(busStopsSet -> busStopsSet
          .stream()
